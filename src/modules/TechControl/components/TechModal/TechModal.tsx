...
const TechModal: FC<TechModalProps> = ({ trigger, tech }) => {
    ...
    return <Modal
        ...
        opened={isOpen}
    >
        <TechModalContent closeParentModal={() => setOpen(false)} tech={tech} />
    </Modal>
}
...
const TechModalContent: FC<TechModalContentProps> = ({ tech, closeParentModal }) => {
    ...
    const [selectOrganization, setSelectOrganization] = useState(tech?.organization.id ?? currentOrgainzationId ?? undefined)
    ...
    return <Stack spacing={2}>
        ...
        <FormControl fullWidth>
            ...
            <Select
                ...
                onChange={(event) => setSelectOrganization(event.target.value)}
            ...
            >
                {
                    organizations.map(({ id, name }) => <MenuItem key={id} value={id}>{name}</MenuItem>)
                }
            </Select>
        </FormControl>
        <FormControl fullWidth>
        ...
        <FormControl fullWidth>
            ...
            >
                {
                    statuses.map(({ id, name }) => <MenuItem key={id} value={id}>{name}</MenuItem>)
                }
            </Select>
        </FormControl>
        <TextField
